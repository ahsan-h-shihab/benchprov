...
friends circulate on the Hugging Face Hub, and most don't say how they were
made. In a structured sample of 96 translated MMLU-family datasets, **65%
declared neither a translation engine nor a human-verification status**
anywhere in their card. Yet the difference matters: harnesses and papers
already act on it when they can —
...
every fact traceable to quoted, revision-pinned evidence.
...
a deliberate low-resource wedge (Kazakh, Russian, Bengali, Uzbek — e.g. all
three of Kazakhstan's independent MMLU translations, none of which names its
engine). The registry's own statistics document the gap it addresses:
**67 of 104 records have no determinable human-verification status; 23
cannot name their translation engine; 12 cannot even establish machine-vs-
human.** Unknown is recorded as unknown — that is the point.
...
```json
{
  ...
  "evidence": [ {"id": "e1", "type": "dataset_card",
                 ...
                 "quote": "Arabic version of MMLU dataset tranlasted by gpt-3.5-turbo."} ],
...
- **`human_verification` is the weakest-evidenced dimension** (67/104
  unknown) — that reflects the ecosystem, not a curation shortcut, but it
  caps what the registry can answer today.
